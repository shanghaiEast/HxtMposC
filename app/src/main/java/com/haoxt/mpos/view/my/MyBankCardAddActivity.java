...
//				toActivity(SettingActivity.createIntent(context));
    ...
    private void getBankOcrResult() {
        ...
        bulider.setBankOCRFinished((bankCardInfo, path) -> {
            /**银行卡识别信息回掉,之后的逻辑业务请自行处理*/
            //e.g.
//            Intent mIntent = new Intent(MyBankCardAddActivity.this, BankCardResultActivity.class);
//            mIntent.putExtra("cardNum", bankCardInfo.cardNum);
//            mIntent.putExtra("bankName", bankCardInfo.bankName);
//            mIntent.putExtra("cardName", bankCardInfo.cardName);
//            mIntent.putExtra("cardType", bankCardInfo.cardType);
//            mIntent.putExtra("cardPath", path);
//            startActivity(mIntent);
            Log.i(TAG, "getBankOcrResult: " + path);
            btn_real_name_auth_debit_card_side.setImageBitmap(ImgUtil.getBitmapByPath(path));
        });
    }
